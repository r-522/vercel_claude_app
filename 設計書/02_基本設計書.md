...
sequenceDiagram
    ...
    Note over UC: modelId/effort/thinking は\nRef 経由でリクエストボディに付加
    UC->>CA: POST /api/chat\n{ messages, modelId, effort, thinking }
    ...
    alt 認証失敗
        ...
        AN-->>CA: トークンストリーム
        CA-->>UC: toUIMessageStreamResponse() (SSE)
        UC-->>CI: messages リアルタイム更新
        CI-->>User: MessageList 再レンダリング
    end
```
...
レスポンス（成功）:
  ...
  Body: AI SDK UIMessageStream プロトコル（toUIMessageStreamResponse() 形式）
