...
Anthropic Claude AI と統合した Web ベースのチャットアプリケーション。4桁アクセスコードによるシンプルな認証を通過したユーザーが、複数の Claude モデルとリアルタイムに対話できる。マークダウンレンダリング・コード強調表示・画像添付・Extended Thinking（思考プロセス表示）など、実用的な研究・執筆支援機能を提供する。さらに GitHub OAuth 連携により、ユーザーは GitHub リポジトリを参照してコンテキスト付きで Claude と対話し、変更を直接 GitHub にプッシュできる Code モードを利用可能。
...
- **GitHub OAuth 2.0 統合**（Code モード）
  - GitHub リポジトリ参照・ブランチ選択
  - ファイルエクスプローラー・ファイルコンテキスト取得
  - ファイル修正の変更追跡・レビュー
  - 新規ブランチ作成と GitHub へのプッシュ
- Vercel へのデプロイ
...
### 4.10 GitHub 統合機能（Code モード）

| 要件ID | 項目 | 詳細 |
|--------|------|------|
| REQ-GH-001 | OAuth 2.0 認証 | GitHub OAuth App によるユーザー認証。`GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET` で設定 |
| REQ-GH-002 | トークン暗号化 | GitHub アクセストークンを `COOKIE_SECRET` で暗号化し HTTP-Only session cookie に保存 |
| REQ-GH-003 | リポジトリ一覧取得 | GitHub API から認証済みユーザーのリポジトリ一覧を取得・表示 |
| REQ-GH-004 | ブランチ選択 | 選択リポジトリのブランチ一覧を取得。デフォルトブランチ自動選択 |
| REQ-GH-005 | ファイルエクスプローラー | 選択ブランチのディレクトリツリーを表示。ファイルをクリックしてコンテキストに追加可能 |
| REQ-GH-006 | ファイルコンテンツ取得 | 選択ファイルのコンテンツを GitHub API から取得し Claude に送信 |
| REQ-GH-007 | コード修正チャット | リポジトリコンテキストを指定して Claude と対話。修正案を受け取り可能 |
| REQ-GH-008 | 変更追跡・レビュー | Claude の修正提案を diff 形式で表示。承認後に変更をステージング |
| REQ-GH-009 | ブランチ作成・プッシュ | 承認した変更を新規ブランチに作成して GitHub にプッシュ |
| REQ-GH-010 | セッション限定 | GitHub トークンはセッション内でのみ保持。ログアウト時に削除 |

### 4.11 Web検索機能（チャットモード）

Anthropic のサーバーサイド Web 検索ツール（`web_search_20250305`）を利用し、Claude が最新情報を検索・引用付きで回答できる機能。検索処理は Anthropic 側で実行されるため、追加の検索 API 契約・キーは不要（既存の `ANTHROPIC_API_KEY` のみ）。

| 要件ID | 項目 | 詳細 |
|--------|------|------|
| REQ-WEB-001 | オン/オフ トグル | モデル設定パネル（`ModelSettings`）に Web検索トグルスイッチを配置。既定はオフ（`DEFAULT_WEB_SEARCH = false`） |
| REQ-WEB-002 | 全モデル対応 | Thinking と異なり、Opus / Sonnet / Haiku すべてで有効化可能（`supportsThinking` のゲーティング対象外） |
| REQ-WEB-003 | サーバー側ツール付与 | `webSearch === true` の場合のみ `streamText` に `anthropic.tools.webSearch_20250305({ maxUses })` を付与。クライアント値が厳密に `true` のときだけ有効化 |
| REQ-WEB-004 | 最大検索回数 | 1 リクエストあたりの検索回数上限を `WEB_SEARCH_MAX_USES`（既定 5）で制限 |
| REQ-WEB-005 | 出典表示 | `toUIMessageStreamResponse({ sendSources: true })` で `source-url` パートを送出。`MessageItem` が「出典」セクションとして URL リンクを重複排除して一覧表示 |
| REQ-WEB-006 | Thinking との併用 | Web検索トグルは Thinking トグルと独立。両方有効でも動作（排他ではない） |
| REQ-WEB-007 | 適用範囲 | 本機能はチャットモード（`/api/chat`）のみ。Code モードは対象外 |
